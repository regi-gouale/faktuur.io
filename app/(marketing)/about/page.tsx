    ...
    <div className="space-y-24 pb-24 pt-24">
      ...
      <section className="relative">
        ...
        <div className="container mx-auto grid gap-10 px-4 lg:grid-cols-[1.1fr_0.9fr] lg:items-center lg:px-8">
          <Card className="border-border/70 bg-background/95 shadow-lg shadow-primary/10">
            <CardHeader>
              <Badge
                variant="outline"
                className="w-fit rounded-full border-border/70 bg-background/80 text-xs font-medium uppercase tracking-wide text-muted-foreground"
              >
                Pourquoi Cotizoo ?
              </Badge>
            ...
            <CardContent className="space-y-4 text-sm leading-6 text-muted-foreground">
              <p>
                Après des années à naviguer entre outils cloisonnés, nos
                fondateurs ont décidé de créer une plateforme qui concilie
                conformité, design et automatisation.
              </p>
              <p>
                Nous collaborons chaque semaine avec un panel
                d&apos;utilisateurs pour valider chaque fonctionnalité, du
                calcul de TVA aux relances intelligentes.
              </p>
              <p>
                Notre promesse : une expérience fluide, que vous soyez en solo,
                en collectif ou à la tête d&apos;une agence qui facture des
                dizaines de clients.
              </p>
            </CardContent>
          </Card>
          <div className="space-y-6">
            {milestones.map((milestone) => (
              <div
                key={milestone.year}
                className="rounded-2xl border border-border/70 bg-background/90 p-6 shadow-sm shadow-primary/5"
              >
      ...
      <section className="container mx-auto px-4 lg:px-8">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-2">
          {values.map((value) => (
            <Card
              key={value.title}
              className="group relative overflow-hidden border border-border/70 bg-background/90 p-6 shadow-sm shadow-primary/5 transition hover:-translate-y-1 hover:border-primary/50 hover:shadow-lg hover:shadow-primary/10"
            >
        ...
        <div className="container mx-auto px-4 lg:px-8">
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {teamMembers.map((member) => (
              <Card
                key={member.name}
                className="border border-border/70 bg-background/95 p-6 text-center shadow-sm shadow-primary/5"
              >
                <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full border border-border/60 bg-muted/40">
                ...
                <CardContent className="px-0 pt-0 text-sm text-muted-foreground">
                  {member.focus}
                </CardContent>
              </Card>
            ))}
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {commitments.map((commitment, index) => (
            <Card
              ...
              className="relative overflow-hidden border border-border/70 bg-background/95 p-6 shadow-sm shadow-primary/5"
            >
              <div className="flex items-center gap-3">
                <Badge className="rounded-full bg-primary/10 text-primary">
                  0{index + 1}
